...
if (process.browser) {
  if (globalThis?.Blob) {
    ...
    const workerBlob = new Blob([threadBytes], {
      type: "application/javascript",
    });
    workerSource = URL.createObjectURL(workerBlob);
  } else {
    workerSource = "data:application/javascript;base64," +
      globalThis.btoa(threadStr);
  }
} else {
  workerSource = "data:application/javascript;base64," +
    Buffer.from(threadStr).toString("base64");
}
...
export default async function buildThreadManager(wasm, singleThread) {
  ...
  ///////////////////////////////////////////////////////////////////////////
  //      POLYFILLS: These are needed for the browser to work in Deno.     //
  ///////////////////////////////////////////////////////////////////////////
  singleThread = true;
