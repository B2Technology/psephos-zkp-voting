...
export default async function builder(code, options) {
  ...
  while (!memoryAllocated) {
    try {
      ...
      console.warn(
        "Could not allocate " + memorySize * 1024 * 64 +
          " bytes. This may cause severe instability. Trying with " +
          memorySize * 1024 * 64 / 2 + " bytes",
      );
    ...
    runtime: {
      ...
      showSharedRWMemory: function () {
        ...
        for (let j = 0; j < shared_rw_memory_size; j++) {
          arr[shared_rw_memory_size - 1 - j] = instance.exports
      ...
      error: function (code, pstr, a, b, c, d) {
        let errStr;
        if (code == 7) {
          errStr = p2str(pstr) + " " + wc.getFr(b).toString() + " != " +
            wc.getFr(c).toString() + " " + p2str(d);
        } else if (code == 9) {
...
class WitnessCalculatorCircom1 {
  ...
  async calculateBinWitness(input, sanityCheck) {
    ...
    const buff = self.memory.buffer.slice(
      pWitnessBuffer,
      pWitnessBuffer + (self.NVars * self.n64 * 8),
    );
